...
/** Deportes distintos de las canchas activas del complejo (para el menú de filtro). */
async function getDeportesComplejo(complexId) {
  const fields = await Field.findAll({ where: { complex_id: complexId, activa: true }, attributes: ['deporte'] });
  const set = new Set(fields.map(f => f.deporte || 'otro'));
  return [...set];
}
 ...
 */
async function getAvailableByHour(fecha, complexId, duracion = 60, deporte = null) {
  const fields = await Field.findAll({
    where: { complex_id: complexId, activa: true },
    order: [['nombre', 'ASC']],
  });
  ...
  for (const field of fields) {
    // Filtro por deporte (si se indicó)
    if (deporte && (field.deporte || 'otro') !== deporte) continue;
 ...
 */
async function getOccupiedByHour(fecha, complexId, duracion = 60, deporte = null) {
  ...
  try {
    ...
    if (msgType === 'interactive' && msg.interactive?.type === 'list_reply') {
      ...
      if (replyId.startsWith('day_')) {
        // Eligió un día → mostrar el menú de DEPORTES (o saltar si hay uno solo)
        const fecha = replyId.replace('day_', '');
        await _sendSportsMenu(ctx, from, fecha);
        return;
      }

      if (replyId.startsWith('dep_')) {
        // Eligió un deporte → mostrar las franjas horarias
        const [fc, deporte] = replyId.replace('dep_', '').split('_');
        await _sendGroupsMenu(ctx, from, fechaFromCompact(fc), deporte);
        return;
      }

      if (replyId.startsWith('grp_')) {
        // Eligió una franja → mostrar el menú de duración
        const [fc, group, deporte] = replyId.replace('grp_', '').split('_');
        await _sendDurationMenu(ctx, from, fechaFromCompact(fc), group, deporte);
        return;
      }

      if (replyId.startsWith('dur_')) {
        // Eligió la duración → mostrar horas libres (franja + duración + deporte)
        const [fc, group, deporte, dur] = replyId.replace('dur_', '').split('_');
        await _sendHoursMenu(ctx, from, fechaFromCompact(fc), group, parseInt(dur), deporte);
        return;
      }

      if (replyId.startsWith('hr_')) {
        // Eligió una hora → mostrar las canchas disponibles (hora + duración + deporte)
        const [fc, deporte, dur, hc] = replyId.replace('hr_', '').split('_');
        const hora = `${hc.slice(0, 2)}:${hc.slice(2)}`;
        await _sendCourtsMenu(ctx, from, fechaFromCompact(fc), hora, parseInt(dur), deporte);
        return;
      }
      ...
      if (replyId.startsWith('wl_')) {
        ...
        const [fc, group, deporte, dur] = replyId.replace('wl_', '').split('_');
        if (!(await waitlist.moduloHabilitado(ctx.clubId))) {
          await send({ to: from, type: 'text', text: { body: 'ℹ️ La función de lista de espera no está disponible actualmente.' } });
          return;
        }
...
/**
 * Menú de DEPORTES disponibles en el complejo (paso posterior a elegir el día).
 *  - Si hay más de un deporte → muestra el menú de selección.
 *  - Si hay exactamente uno → omite el paso y sigue directo a las franjas.
 */
async function _sendSportsMenu(ctx, to, fecha) {
  const send = p => wa.sendMessage(p, ctx.creds);
  const deportes = await getDeportesComplejo(ctx.clubId);

  if (!deportes.length) {
    await send({ to, type: 'text', text: { body: '😔 Este complejo todavía no tiene canchas activas.' } });
    return;
  }
  // Un solo deporte → saltear el filtro y continuar con las franjas.
  if (deportes.length === 1) {
    await _sendGroupsMenu(ctx, to, fecha, deportes[0]);
    return;
  }

  const fc = fechaCompact(fecha);
  const rows = deportes.map(d => ({
    id:          `dep_${fc}_${d}`,
    title:       `${depEmoji(d)} ${labelDeporte(d)}`.substring(0, 24),
    description: 'Ver turnos de este deporte',
  }));

  await send(wa.buildRowsListMessage(to, {
    headerText:   `🏅 ${formatFechaLabel(fecha)}`,
    bodyText:     '¿Qué deporte querés jugar?',
    footerText:   'JugaHoy — Reservas deportivas',
    button:       'Ver deportes',
    sectionTitle: 'Deportes disponibles',
    rows,
  }));
}

/**
 * Menú de franjas horarias (mañana / tarde / noche) para una fecha + deporte.
 ...
 */
async function _sendGroupsMenu(ctx, to, fecha, deporte) {
  const send = p => wa.sendMessage(p, ctx.creds);
  const fc = fechaCompact(fecha);
  const dep = deporte || '';
  ...
  const depTxt = deporte ? ` · ${depEmoji(deporte)} ${labelDeporte(deporte)}` : '';
  await send(wa.buildRowsListMessage(to, {
    headerText:   `🕐 ${formatFechaLabel(fecha)}${depTxt}`.substring(0, 60),
...
/** Menú de duración del turno (1 h / 1½ h / 2 h) para una franja + deporte */
async function _sendDurationMenu(ctx, to, fecha, group, deporte) {
  const send = p => wa.sendMessage(p, ctx.creds);
  const franja = FRANJAS[group];
  const fc = fechaCompact(fecha);
  const dep = deporte || '';
  const rows = DURACIONES.map(d => ({
    id:          `dur_${fc}_${group}_${dep}_${d.min}`,
    title:       d.label,
    description: `${d.min} minutos`,
  }));
  await send(wa.buildRowsListMessage(to, {
    headerText:   `⏱️ ${formatFechaLabel(fecha)} · ${franja?.label || ''}`.substring(0, 60),
...
/** Menú de horas libres dentro de una franja+duración+deporte, con sus canchas */
async function _sendHoursMenu(ctx, to, fecha, group, duracion, deporte) {
  const send = p => wa.sendMessage(p, ctx.creds);
  const franja = FRANJAS[group];
  if (!franja) return _sendGroupsMenu(ctx, to, fecha, deporte);
  const dep = deporte || '';

  const byHour = await getAvailableByHour(fecha, ctx.clubId, duracion, deporte);
  ...
  const occByHour = await getOccupiedByHour(fecha, ctx.clubId, duracion, deporte);
  const hayOcupados = Object.keys(occByHour).some(h => franja.test(parseInt(h)));

  const fc = fechaCompact(fecha);
  const waitlistRow = {
    id:          `wl_${fc}_${group}_${dep}_${duracion}`,
  ...
  if (!horas.length) {
    ...
    if (hayOcupados) {
      await send(wa.buildRowsListMessage(to, {
        headerText:   `⏰ ${formatFechaLabel(fecha)} · ${franja.label}`.substring(0, 60),
    ...
    await _sendDurationMenu(ctx, to, fecha, group, deporte);
    return;
  }

  let rows = horas.map(hora => ({
    id:          `hr_${fc}_${dep}_${duracion}_${hora.replace(':', '')}`,
    title:       `${hora} hs`,
...
async function _sendWaitlistMenu(ctx, to, fecha, group, duracion, deporte) {
  ...
  // Turnos que el usuario YA tiene en lista de espera (para no repetirlos).
  const yaAnotado = await waitlist.slotsActivosDeTelefono(ctx.clubId, to).catch(() => new Set());
...
/** Menú de canchas disponibles para una hora + duración + deporte concretos */
async function _sendCourtsMenu(ctx, to, fecha, hora, duracion, deporte) {
  const send = p => wa.sendMessage(p, ctx.creds);
  const byHour = await getAvailableByHour(fecha, ctx.clubId, duracion, deporte);
  ...
  if (!courts.length) {
    ...
    await _sendGroupsMenu(ctx, to, fecha, deporte);
    return;
  }
  ...
  await send(wa.buildRowsListMessage(to, {
    headerText:   `🏟️ ${formatFechaLabel(fecha)} · ${hora} hs (${duracionLabel(duracion)})`.substring(0, 60),
...
async function _handleConfirm(ctx, from, slotRaw) {
  ...
  try {
    ...
    if (ocupados.length > 0) {
      ...
      await _sendGroupsMenu(ctx, from, fecha, field?.deporte);
      return;
    }
